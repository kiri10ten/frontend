...
<template>
    ...
    <div class="bg-custom-grey pt-4 flex flex-col   rounded-md">
        ...
        <div class="flex flex-row  justify-between pl-6 pt-3 pb-7 pr-6 gap-5">
                ...
                <div class="pt-2">
                <p class="text-gray-400 text-sm">Author: {{ archive.created_by_initials }}</p>
                </div>
                  <div class="pt-2">
                  <p class="text-gray-400 text-sm"> {{ archive.created_at_formatted }}</p>
            ...
            <div class="flex gap-6 justify-end">
                ...
                <NuxtLink v-bind:to="'/editjob/' + archive.id" v-if="my" class="bg-green-200  text-gray-800 rounded-sm text-sm  p-2">
                    Edit
                </NuxtLink>
                <a @click="deleteArchive(archive.id)" v-if="my" class=" bg-red-200 text-gray-800 rounded-sm text-sm  p-2">
                    Delete
                </a>
            </div>
        </div>
